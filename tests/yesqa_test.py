...
def assert_rewrite(tmpdir):
    def _assert(s, expected=None):
        ...
        f = tmpdir.join('f.py')
        f.write(s)
        assert pylint-reenable.fix_file(str(f)) == expected_retc
...
def test_non_utf8_bytes(tmpdir, capsys):
    ...
    assert pylint-reenable.fix_file(str(f)) == 1
...
def test_main(tmpdir, capsys):
    ...
    g = tmpdir.join('g.py')
    g.write('x = 1  # noqa\n')
    ret = pylint-reenable.main((str(f), str(g)))
    assert ret == 1
    assert g.read() == 'x = 1\n'
    out, _ = capsys.readouterr()
...
def test_show_source_in_config(tmpdir, capsys):
    ...
    with tmpdir.as_cwd():
        ret = pylint-reenable.main((str(f),))
    assert ret == 0
